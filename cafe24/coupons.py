    ...
    with open("couponissues.csv", 'w', encoding='utf-8-sig') as couponissues:
        ...
        for i in range(0,len(couponid)):
            ...
            if issuedcount[i] <= 8000:
                ...
                sinceissueno = "&since_issue_no=0000000000000000000"

            while True:
                for k in range(0,offset,500):
                    while True:
                        url = "https://instinctus1.cafe24api.com/api/v2/admin/coupons/%s/issues?limit=500&offset=%s%s%s" %(couponid[i],k,searchdate,sinceissueno)
                        response = requests.request("GET", url, headers=headers)
                        time.sleep(0.45)
                        apicount += 1
                        try:
                            ...
                            print(response.text)
                        if len(couponissues_json) == 500 and 8000 < issuedcount[i]:
                            sinceissueno = "&since_issue_no=" + couponissues_json[-1]['issue_no']
                        else:
                            break
                if onecouponissuecount < issuedcount[i] and start_date != None and start_date < time1.date():
